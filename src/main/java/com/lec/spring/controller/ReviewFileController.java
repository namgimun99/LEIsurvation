...
import com.lec.spring.service.ReviewFileService;
...
public class ReviewFileController {
    ...
    @Value("${app.review.upload.path}")  // org.springframework.beans.factory.annotation.Value
    private String uploadDir;

    private ReviewFileService fileService;

    @Autowired
    public void setFileService(ReviewFileService fileService) {
        this.fileService = fileService;
    }

    public ReviewFileController() {
        System.out.println(getClass().getName() + "() 생성");
    }

    // 파일 다운로드
    // id : 첨부파일의 id
    @RequestMapping("/review/download")
    public ResponseEntity<Object> download(Long id){

        if(id == null) return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);

        ReviewFileDTO file = fileService.findById(id);
